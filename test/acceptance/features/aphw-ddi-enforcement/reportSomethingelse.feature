Feature: Report something else
    ...
    Scenario: Search for the dog
        When I set "Bunty" to the inputfield "#searchTerms"
        And I press "Enter"
    ...
    Scenario: Report something else
        When I click on the link "Report something"
        And I select the radio option with the value "something-else" from the radio group "reportType"
        And I click on the element "button=Continue"
        And I set "Test data" to the inputfield "#details"
        And I click on the element "button=Continue"
